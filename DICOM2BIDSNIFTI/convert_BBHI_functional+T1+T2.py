...
#heudiconv -d /institut/BBHI_DICOMS/{subject}/*/*IMA -s 162766 -ss 01 -f /home/didac/Scripts/dicom2BIDS/convert_BBHI_functional+T1+T2.py  -c dcm2niix -b -o /institut/processed_data/BBHI_func 
...
#                info[pcasl_epi_pa].append(s.series_id)
        ...
        #resting
        if s.protocol_name=='RESTING':
            if s.sequence_name=='epse2d1_104':
                info[rest_sefm_ap].append(s.series_id)
            elif s.sequence_name=='epfid2d1_104':
                 if s.image_type[3]=='MB':             
                    info[rest_bold_ap].append(s.series_id)
                 elif  s.image_type[3]=='ND':               
                    info[rest_sbref_ap].append(s.series_id)
        elif s.protocol_name=='RESTING_PA':
            if s.sequence_name=='epse2d1_104':
                info[rest_sefm_pa].append(s.series_id)                
            elif s.sequence_name=='epfid2d1_104':
                 if s.image_type[3]=='MB':             
                    info[rest_bold_pa].append(s.series_id)
                 elif  s.image_type[3]=='ND':               
                    info[rest_sbref_pa].append(s.series_id)  
