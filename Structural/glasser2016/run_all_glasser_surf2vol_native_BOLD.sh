...
for i in $list_subs; do
	...
	if [[ $SUBJECT_ID == *"sub"* ]]; then #to avoid folders that are not for a subject, e.g. /fsaverage
		if [[ $done_subs_list =~ (^|[[:space:]])$SUBJECT_ID($|[[:space:]]) ]]; then
			...
			bash glasser_surf2vol_native_BOLD.sh --subjects_dir $SUBJECTS_DIR --subject_id $SUBJECT_ID --left_annotation $GLASSER_FSAVERAGE_LEFT --right_annotation $GLASSER_FSAVERAGE_RIGHT --output_dir $OUTPUT_FOLDER
		fi
	fi
