...
for i in $list_subs; do
	...
	if [[ $SUBJECT_ID == *"sub"* ]]; then #to avoid folders that are not for a subject, e.g. /fsaverage
		if [[ $done_subs_list =~ (^|[[:space:]])$SUBJECT_ID($|[[:space:]]) ]]; then
			echo $separate
			echo "Glasser and subcortical atlas already obtained for $SUBJECT_ID"
		else 
			echo "Obtaining Glasser and Subcortical Atlas for $SUBJECT_ID"
			...
			bash /home/mariacabello/git_projects/MRI_preprocess/Structural/subcortical14.sh --subjects_dir $SUBJECTS_DIR --subject_id $SUBJECT_ID --output_dir $OUTPUT_FOLDER
		fi
	fi
